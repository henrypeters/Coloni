...
let itemsArray = localStorage.getItem('drawCom1Items') ? JSON.parse(localStorage.getItem('drawCom1Items')) : [];
...
function add(){
    if (input.value.length > 0) {
        ...
        localStorage.setItem('drawCom1Items', JSON.stringify(itemsArray));
...
function del(){
  localStorage.removeItem('drawCom1Items');
  todoCont.innerHTML =  '';
  itemsArray = [];
  input.value = ''
}