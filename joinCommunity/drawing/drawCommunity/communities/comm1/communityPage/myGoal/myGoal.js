...
let itemsArrayMyGoal = localStorage.getItem('drawItemsMyGoal') ? JSON.parse(localStorage.getItem('drawItemsMyGoal')) : [];
...
function add(){
    if (input.value.length > 0) {
        ...
        localStorage.setItem('drawItemsMyGoal', JSON.stringify(itemsArrayMyGoal));
...
function del(){
  localStorage.removeItem('foodItemsMyGoal');
  todoCont.innerHTML =  '';
  itemsArrayMyGoal = [];
  input.value = ''
}