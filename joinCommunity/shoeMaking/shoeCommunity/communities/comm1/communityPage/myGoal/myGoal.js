...
let itemsArrayMyGoal = localStorage.getItem('itemsMyGoal') ? JSON.parse(localStorage.getItem('itemsMyGoal')) : [];
...
function add(){
    if (input.value.length > 0) {
        ...
        localStorage.setItem('itemsMyGoal', JSON.stringify(itemsArrayMyGoal));
...
function del(){
  localStorage.removeItem('itemsMyGoal');
  todoCont.innerHTML =  '';
  itemsArrayMyGoal = [];
  input.value = ''
}
