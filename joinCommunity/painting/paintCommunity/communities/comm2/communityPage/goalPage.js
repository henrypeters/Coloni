...
let itemsArray = localStorage.getItem('paintCom2Items') ? JSON.parse(localStorage.getItem('paintCom2Items')) : [];
...
function add(){
    if (input.value.length > 0) {
        ...
        localStorage.setItem('paintCom2Items', JSON.stringify(itemsArray));
...
function del(){
  localStorage.removeItem('paintCom2Items');
  todoCont.innerHTML =  '';
  itemsArray = [];
  input.value = ''
}