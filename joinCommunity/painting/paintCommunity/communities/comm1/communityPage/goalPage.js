...
let itemsArray = localStorage.getItem('paintCom1Items') ? JSON.parse(localStorage.getItem('paintCom1Items')) : [];
...
function add(){
    if (input.value.length > 0) {
        ...
        localStorage.setItem('paintCom1Items', JSON.stringify(itemsArray));
...
function del(){
  localStorage.removeItem('paintCom1Items');
  todoCont.innerHTML =  '';
  itemsArray = [];
  input.value = ''
}