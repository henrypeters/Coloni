...
let itemsArray = localStorage.getItem('foodCom1Items') ? JSON.parse(localStorage.getItem('foodCom1Items')) : [];
...
function add(){
    if (input.value.length > 0) {
        ...
        localStorage.setItem('foodCom1Items', JSON.stringify(itemsArray));
...
function del(){
  localStorage.removeItem('foodCom1Items');
  todoCont.innerHTML =  '';
  itemsArray = [];
  input.value = ''
}