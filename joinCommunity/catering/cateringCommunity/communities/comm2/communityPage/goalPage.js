...
let itemsArray = localStorage.getItem('foodCom2Items') ? JSON.parse(localStorage.getItem('foodCom2Items')) : [];
...
function add(){
    if (input.value.length > 0) {
        ...
        localStorage.setItem('foodCom2Items', JSON.stringify(itemsArray));
...
function del(){
  localStorage.removeItem('foodCom2Items');
  todoCont.innerHTML =  '';
  itemsArray = [];
  input.value = ''
}